"""Greenhouse job application adapter."""
...
def is_greenhouse(page: "Page") -> bool:
    """Check if the current page is a Greenhouse application form."""
    try:
        url = page.url.lower()
            ...
            try:
                ...
                for pattern in yes_no_patterns:
                    if pattern.lower() in label_text:
                        ...
                        # Look for radio buttons or checkboxes associated with this label
                        try:
                            # Check if there's a "Yes" option nearby
...
def apply(
    ...
    try:
        ...
        # Greenhouse often has a cover letter field
        ...
        if per_site_applied is not None:
            per_site_applied["greenhouse"] = per_site_applied.get("greenhouse", 0) + 1
...
class GreenhouseAdapter:
    """Greenhouse adapter implementing JobPlatformAdapter. No search-based discovery."""
    ...
    def apply_to_job(
        ...
        if not is_greenhouse(page):
            ...
            ctx.log(entry)
            return ApplicationResult(status=STATUS_ERROR, notes="not_a_greenhouse_page")
        ...
        try:
            ...
            return ApplicationResult(
                status=STATUS_APPLIED, notes="", site_applied="greenhouse"
            )
